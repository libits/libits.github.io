...
![image-20230812225336223](/assets/blog_res/2023-07-07-sol.assets/image-20230812225336223.png)
...
![image-20230806231920054](../../assets/blog_res/2023-07-07-sol.assets/image-20230806231920054.png)
...
- 映射可以标记为`public`，Solidity 自动为它创建getter。
- mapping 不能直接在函数返回
...
- 执行交易的指令要收取汽油费用，由发起交易的人来支付
- 智能合约不存在try-catch结构
- 一旦遇到异常 除非特别情况 否则本次执行操作全部回滚，但是已经耗掉的gas费用不退
- 可以抛出错误的语句：assert(bool condition)：抛出内部错误；require(bool condition):抛掉用于输入或者外部组件引起的错误
- revert():终止运行并回滚状态变动
...
###  fallback()函数:function() public [payable]{}

- 匿名函数，没有参数也没有返回值
- 会被调用的情况：直接向一个合约地址转账，而不加任何data；被调用的函数不存在
